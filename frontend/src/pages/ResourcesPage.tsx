...
function nodeColor(state: string): string {
  ...
  if (s.includes('down') || s.includes('drain')) return 'bg-gray-600'
  return 'bg-gray-500'
}
...
function NodeGrid({ nodes }: { nodes: NodeInfo[] }) {
  ...
  return (
    <div className="bg-surface-1 rounded-xl border border-white/5 p-5">
      <h3 className="text-sm font-medium text-gray-300 mb-3">Node Map</h3>
      <div className="flex gap-2 mb-3 text-xs text-gray-400">
        ...
        <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded bg-gray-600" /> Down</span>
      </div>
      <div className="flex flex-wrap gap-1">
        {nodes.slice(0, 200).map(n => (
          <div
            key={n.name}
            className={`w-3 h-3 rounded-sm cursor-pointer ${nodeColor(n.state)} opacity-80 hover:opacity-100 transition-opacity`}
      ...
      {nodes.length > 200 && <p className="text-xs text-gray-500 mt-2">Showing 200 of {nodes.length} nodes</p>}

      {hovered && (
        <div className="mt-3 p-3 bg-surface-2 rounded-lg text-xs">
          <div className="font-mono text-gray-200 mb-1">{hovered.name}</div>
          <div className="grid grid-cols-3 gap-x-4 text-gray-400">
            <span>State: <span className="text-gray-200">{hovered.state}</span></span>
            <span>CPUs: <span className="text-gray-200">{hovered.cpus_alloc}/{hovered.cpus_total}</span></span>
            <span>GPUs: <span className="text-gray-200">{hovered.gpus_alloc}/{hovered.gpus_total}</span></span>
            <span>Mem: <span className="text-gray-200">{Math.round(hovered.memory_alloc / 1024)}G / {Math.round(hovered.memory_total / 1024)}G</span></span>
            <span className="col-span-2">Partitions: <span className="text-gray-200">{hovered.partitions.join(', ') || '-'}</span></span>
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div className="bg-surface-1 rounded-xl border border-white/5 p-5">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-medium text-gray-300">Usage History</h3>
        <div className="flex gap-1">
          {[6, 12, 24, 48].map(h => (
            <button key={h} onClick={() => setHours(h)}
              className={`px-2 py-1 text-xs rounded ${hours === h ? 'bg-accent/20 text-accent' : 'text-gray-400 hover:text-gray-200'}`}>
              {h}h
            </button>
          ))}
        </div>
      ...
      <ResponsiveContainer width="100%" height={250}>
        <AreaChart data={chartData}>
          ...
          <Tooltip
            contentStyle={{ backgroundColor: '#1a2235', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '8px' }}
            labelStyle={{ color: '#9ca3af' }}
          />
          <Area type="monotone" dataKey="cpu" stroke="#3b82f6" fill="#3b82f6" fillOpacity={0.1} name="CPU %" />
          <Area type="monotone" dataKey="gpu" stroke="#6366f1" fill="#6366f1" fillOpacity={0.1} name="GPU %" />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  )
...
function PartitionTable() {
  ...
  return (
    <div className="bg-surface-1 rounded-xl border border-white/5 p-5">
      <h3 className="text-sm font-medium text-gray-300 mb-3">Partitions</h3>
        ...
        <tbody>
          {partitions.map(p => (
            <tr key={p.name} className="border-b border-white/[0.03]">
              <td className="py-2 text-gray-300 font-mono">{p.name}</td>
              <td className="py-2 text-right text-gray-400">{p.total_nodes}</td>
              <td className="py-2 text-right text-success">{p.available_nodes}</td>
              <td className="py-2 text-right text-gray-400">{p.allocated_cpus}/{p.total_cpus}</td>
            </tr>
          ))}
        </tbody>
      </table>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          <div key={label} className="bg-surface-1 rounded-xl border border-white/5 p-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm text-gray-400">{label}</span>
              <span className="text-sm text-gray-200">{used} / {total}</span>
            </div>
